...
| ✅ | **U-TE — FDA Orange Book therapeutic-equivalence** | Ingested `products.txt` → `orange_products` (48.5k rows, indexed). `orange_book.te_status_for` returns the FDA `TE_Code`; **independent validation caught a safety overstatement** — the first cut reported flat `substitutable=True` over mixed AB1/AB2/AB3. **Fixed:** per-subletter subgroups + `substitution_scope=subletter_scoped` + `subletter_warning`; group RLD brands (metformin ER → FORTAMET/GLUCOPHAGE XR/GLUMETZA); DISCN/single-source correct (aspirin not substitutable). Candidate `therapeutic_equivalence` **evidence** block (no re-rank/gate — HITL); different-ingredient candidates flagged `applies_to=candidate_own_generics_only`. DQ2 `orange_book_gold_standard` = regulatory relevant-set (ingredient-wide as wired — coarse; exposed not yet scored). Crosswalk (a); 73% OB-ingredient coverage. **Regulatory TE backbone (MCS = chemical support).** |
| ✅ | **U6 — data-driven salt-awareness** | Replaced the hardcoded ~16-ingredient `_MOIETY_FORMS` dict with a **DrugBank-derived `salt_forms` table** (**4,956 rows = 2,292 base-identity + 2,664 salt forms**, over 2,292 base ingredients). `resolve_moiety` resolves salts via source data (`salt_map_drugbank`, 0.9); curated 16 = override; heuristic fallback; graceful when absent. **Two validation rounds:** (1) SAFETY — DrugBank's `salt_forms` also lists non-salts (NPH insulin under insulin human, argipressin), which made them read as "same drug"; builder now ingests only genuine `<base> <salt>` counter-ions → NPH insulin ≢ regular insulin. (2) that filter over-dropped genuine **acid↔ate conjugates** (valproate↔valproic, clavulanate↔clavulanic) — recovered via a shared-root conjugate rule that still excludes the different-word non-salts. Build/query normalisation unified. **Honest scope:** clears the *hardcoded-map* half of D3-10; the eligibility test is still **name-based** (not the spec's MCS/graph mechanism — U5/U9 are the complements). Decision-support only. |
| ✅ | **O3/DQ2 — RDKit-MCS + graph (U4+U5+U9)** | **U4:** catalogue `smiles_by_name` table (59.7k, salt-normalized) + reproducible builder. **U5:** `rdkit==2026.3.5` installed (no segfault, unlike torch); real MCS atom-coverage → bounded score bonus (amox/ampicillin **0.96**≥0.9; metformin/atorvastatin 0.05). **U9:** NetworkX DiGraph **re-documented** (relational-catalogue substitute, A10-style). Gated by `ENABLE_SPEC_MCS` (default on). **Independent validation found + fixed 2 real defects:** (1) the DQ2 MCS ranking was a **P@K/R@K no-op** (valids always rank first) → now surfaced as a distinct `mean_mcs_atom_coverage` metric; (2) salt-strip could annihilate salt-only names / mis-resolve combinations → hardened (combinations → None). Version-pin "mismatch" was a false alarm (2026.03.5 == pin). |
...
| Unit | Decision |
|---|---|
| U-TE | ~~Crosswalk strategy (a) vs (b)?~~ **RESOLVED → strategy (a)** (Ingredient+Form+Route+Strength; 73% of OB ingredients covered — probed). Still pending: document Orange Book as an **A10-style scope addition** (supervisor sign-off, since the spec named RDKit-MCS not Orange Book). |
...
| **U-TE Orange Book TE layer** | D3-02 (regulatory TE), R28, DQ2 gold std (D7-03/04) | Fix + Doc | L–XL | ✅ **done — validated** — 48.5k `orange_products`; `te_status_for` (metformin AB1/AB2/AB3 subletters, DISCN, single-source correct); `therapeutic_equivalence` on candidates (evidence-only); DQ2 `orange_book_gold_standard` (regulatory relevant-set). Crosswalk (a): 73% of OB ingredients covered. |
| U4 SMILES ingest | D3-04, D4-04 | Fix | L | ✅ **done** — `smiles_by_name` table (59.7k, salt-normalized) + reproducible builder |
| U5 RDKit MCS | D3-02, D3-03 | Fix | L | ✅ **done — validated** — rdkit installed; real atom-coverage bonus (amox/ampicillin 0.96). DQ2: MCS surfaced as `mean_mcs_atom_coverage` (P@K/R@K unchanged by design — validation caught the ranking-metric no-op). Salt-strip hardened. |
| U6 Salt-awareness | D3-10 *(salt-map half; MCS mechanism = U5)*, D4-03 *(data; graph = U9)* | Fix | M | ✅ **done — 2× validated** — DrugBank `salt_forms` table (**4,956 rows = 2,292 base-identity + 2,664 salt forms**) replaces the hardcoded 16-dict; `resolve_moiety` uses it (`salt_map_drugbank`, 0.9) + 16-dict override + heuristic. **Validation fixed: (safety)** non-salts (NPH insulin ≡ insulin human, argipressin) excluded; **(over-correction)** genuine acid↔ate conjugates (valproate↔valproic, clavulanate↔clavulanic) recovered via shared-root rule; build/query normalisation unified. Still name-based (not MCS/graph). |
