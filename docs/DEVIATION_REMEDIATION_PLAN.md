...
| ✅ | **U6 — data-driven salt-awareness** | Replaced the hardcoded ~16-ingredient `_MOIETY_FORMS` dict with a **DrugBank-derived `salt_forms` table** (**4,901 salt-only forms / 2,292 base ingredients**). `resolve_moiety` resolves salts via source data (`salt_map_drugbank`, 0.9); curated 16 = override; heuristic fallback; graceful when absent. **Independent validation caught a SAFETY issue & fixed it:** DrugBank's `salt_forms` also lists non-salts (NPH insulin under insulin human) — builder now ingests only genuine `<base> <salt>` counter-ions, so NPH insulin ≢ regular insulin (was wrongly "same drug"). Also unified build/query normalisation (1.8% rows were unreachable). **Honest scope:** clears the *hardcoded-map* half of D3-10; the eligibility test is still **name-based** (not the spec's MCS/graph mechanism — U5/U9 are the complements). Decision-support only. |
...
| U6 Salt-awareness | D3-10 *(salt-map half; MCS mechanism = U5)*, D4-03 *(data; graph = U9)* | Fix | M | ✅ **done — validated** — DrugBank `salt_forms` table (4,901 salt-only forms / 2,292 bases) replaces the hardcoded 16-dict; `resolve_moiety` uses it (`salt_map_drugbank`, 0.9) + 16-dict override + heuristic. **Validation fixed a safety issue:** non-salt DrugBank entries (NPH insulin ≡ insulin human) excluded — genuine `<base> <salt>` only; build/query normalisation unified. Still name-based (not MCS/graph). |
