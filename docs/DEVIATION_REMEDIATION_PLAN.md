...
| ✅ | **U6 — data-driven salt-awareness** | Replaced the hardcoded ~16-ingredient `_MOIETY_FORMS` dict with a **DrugBank-derived `salt_forms` table** (5,248 salt forms across **2,292 base ingredients**, ~140× the coverage). `resolve_moiety` now resolves salts via source data (`salt_map_drugbank`, conf 0.9); the curated 16 remain a hand-verified override, heuristic salt-strip as last-resort fallback (graceful when the table is absent). Verified: clopidogrel/quetiapine/bupropion/amlodipine-camsylate salts (all outside the 16) resolve; cross-salt same-moiety pairs match. Decision-support only (HITL). Reproducible via `scripts/build_salt_forms.py`. |
...
| U6 Salt-awareness | D3-10, D4-03 | Fix | M | ✅ **done** — DrugBank `salt_forms` table (5,248 forms / 2,292 bases) replaces the hardcoded 16-dict; `resolve_moiety` uses it (`salt_map_drugbank`, conf 0.9) with the 16-dict as curated override + heuristic fallback. Salt pairs (amlodipine besylate≡maleate, clopidogrel bisulfate≡besilate) resolve same-moiety. |
