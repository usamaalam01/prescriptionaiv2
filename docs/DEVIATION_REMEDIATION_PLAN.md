...
| ✅ | **U-TE — FDA Orange Book therapeutic-equivalence** | Ingested `products.txt` → `orange_products` (48.5k rows, indexed). `orange_book.te_status_for` returns the FDA `TE_Code` with the **subletter safety rule** (metformin AB1/AB2/AB3 kept distinct), RLD reference, DISCN filter, single-source detection (aspirin correctly *not* substitutable). Surfaced as a `therapeutic_equivalence` **evidence** block on every candidate (no re-rank/gate — HITL). DQ2 gains `orange_book_gold_standard` — a **regulatory relevant-set** (A-rated PE group) closing D7-03/04's "no defensible ground truth". Crosswalk strategy (a); 73% of OB ingredients catalogue-covered. **This is the regulatory TE backbone (MCS = chemical support).** 3 validation rounds pending. |
